...
class EnhancedAgentMonitor:
    ...
    def __init__(
        ...
        """
        Args:
            llm: Optional LLM instance (not required). If not provided,
        ...
        # Initialize LLM for scoring (expect a callable or None)
        # If `llm` is provided it may be ignored in favor of using `llama_call` directly
        # but we still keep the attribute for backward compatibility.
        self.llm = llm
        if not self.llm and debug:
            print("[WARNING] No LLM provided - LLM scoring disabled (falling back to heuristics)")
